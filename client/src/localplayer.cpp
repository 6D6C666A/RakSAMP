...
void SendInCarFullSyncData(INCAR_SYNC_DATA *picSync, int iUseCarPos, PLAYERID followPlayerID)
{
	...
	if(dwLastInVehicleDataSentTick && dwLastInVehicleDataSentTick < (GetTickCount() - iNetModeNormalIncarSendRate))
	{
		if(followPlayerID != (PLAYERID)-1)
		{
			...
			float inFrontX, inFrontY;
			inFrontX = playerInfo[followPlayerID].incarData.vecPos[0];
			inFrontY = playerInfo[followPlayerID].incarData.vecPos[1];
			getXYInFrontOfPlayer(followPlayerID, &inFrontX, &inFrontY, settings.fFollowDistance);

			picSync->vecPos[0] = inFrontX;
			picSync->vecPos[1] = inFrontY;
