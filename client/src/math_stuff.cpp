...
void getXYInFrontOfPlayer(PLAYERID playerID, float *fX, float *fY, float fDistance)
{
	CVector front;
	float f1,f2,f3,f4,f5,f6,f7,f8,f9, fRotation;

	if(playerInfo[playerID].incarData.VehicleID == (VEHICLEID)-1)
		MakeOrthMatrix(
	...
	fRotation = front.GetAngleDegrees();

	float r = (M_PI / 180.0f);
	*fX += (fDistance * sin(fRotation * r));
	*fY += (fDistance * cos(fRotation * r));
}
